...
static GFX_Image logo1, logo2, logo3;
...
static void display() {
  gfx_image_display(&logo1, logo1_position_center.x, logo1_position_center.y);
  gfx_image_display(&logo2, logo2_position_center.x, logo2_position_center.y);
  gfx_image_display(&logo3, logo3_position_center.x, logo3_position_center.y);
...
static void update_logo1() { // Horizontal
  ...
  gfx_image_shrunk_centroid(&logo1, (short)logo1_position_center.x, (short)logo1_position_center.y, shrunk_forge(logo1_shrunk_x, 0xFF));
  if (logo1_shrunk_x == 0) {
    logo1_shrunk_inc = 1;
...
static void update_logo2() { // Proportional
  gfx_image_shrunk_centroid(&logo2, (short)logo2_position_center.x, (short)logo2_position_center.y, get_shrunk_proportional_table(shrunkTableIndex));
  shrunkTableIndex++;
...
static void update_logo3() { // Vertical
  ...
  gfx_image_shrunk_centroid(&logo3, (short)logo3_position_center.x, (short)logo3_position_center.y, shrunk_forge(0XF ,logo3_shrunk_y));
  if (logo3_shrunk_y == 0) {
    logo3_shrunk_inc = 1;
