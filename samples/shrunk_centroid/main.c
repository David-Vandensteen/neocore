...
static void display() {
  display_gfx_picture(&logo1, logo1_position_center.x, logo1_position_center.y);
  display_gfx_picture(&logo2, logo2_position_center.x, logo2_position_center.y);
  display_gfx_picture(&logo3, logo3_position_center.x, logo3_position_center.y);
  log_info("HORIZONTAL SHRUNK");
  set_position_log(1, 11);
  log_info("VERTICAL SHRUNK");
  set_position_log(1, 20);
...
static void update_logo1() { // Horizontal
  ...
  shrunk_centroid_gfx_picture(&logo1, (short)logo1_position_center.x, (short)logo1_position_center.y, shrunk_forge(logo1_shrunk_x, 0xFF));
  if (logo1_shrunk_x == 0) {
    logo1_shrunk_inc = 1;
...
static void update_logo2() { // Proportional
  shrunk_centroid_gfx_picture(&logo2, (short)logo2_position_center.x, (short)logo2_position_center.y, get_shrunk_proportional_table(shrunkTableIndex));
  shrunkTableIndex++;
...
static void update_logo3() { // Vertical
  ...
  shrunk_centroid_gfx_picture(&logo3, (short)logo3_position_center.x, (short)logo3_position_center.y, shrunk_forge(0XF ,logo3_shrunk_y));
  if (logo3_shrunk_y == 0) {
    logo3_shrunk_inc = 1;
