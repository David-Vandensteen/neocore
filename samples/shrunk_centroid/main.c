...
static void display() {
  nc_gfx_display_picture(&logo1, logo1_position_center.x, logo1_position_center.y);
  nc_gfx_display_picture(&logo2, logo2_position_center.x, logo2_position_center.y);
  nc_gfx_display_picture(&logo3, logo3_position_center.x, logo3_position_center.y);
  nc_log_info("HORIZONTAL SHRUNK");
  nc_log_set_position(1, 11);
  nc_log_info("VERTICAL SHRUNK");
  nc_log_set_position(1, 20);
  nc_log_info("PROPORTIONAL SHRUNK");
}

static void update() {
  if (nc_gpu_get_frame_number() % 5 == 0) update_logo1();
  update_logo2();
  update_logo3();
}
...
static void update_logo1() { // Horizontal
  ...
  nc_gpu_shrunk_centroid_gfx_picture(&logo1, (short)logo1_position_center.x, (short)logo1_position_center.y, nc_shrunk_forge(logo1_shrunk_x, 0xFF));
  if (logo1_shrunk_x == 0) {
    logo1_shrunk_inc = 1;
...
static void update_logo2() { // Proportional
  nc_gpu_shrunk_centroid_gfx_picture(&logo2, (short)logo2_position_center.x, (short)logo2_position_center.y, nc_gpu_get_shrunk_proportional_table(shrunkTableIndex));
  shrunkTableIndex++;
...
static void update_logo3() { // Vertical
  ...
  nc_gpu_shrunk_centroid_gfx_picture(&logo3, (short)logo3_position_center.x, (short)logo3_position_center.y, nc_shrunk_forge(0XF ,logo3_shrunk_y));
  if (logo3_shrunk_y == 0) {
    logo3_shrunk_inc = 1;
