...
void player_update() {
  ...
  update_joypad(0);
  if (joypad_is_up(0) && get_y_gfx_animated_sprite_physic(player_sprite) > get_player_min_y()) {
  ...
  if (joypad_is_down(0) && get_y_gfx_animated_sprite_physic(player_sprite) < get_player_max_y()) {
    move_gfx_animated_sprite_physic(&player_sprite, 0, get_player_speed());
    set_animation_gfx_animated_sprite_physic(&player_sprite, PLAYER_IMG_ANIM_DOWN);
  }
  if (joypad_is_left(0) && get_x_gfx_animated_sprite_physic(player_sprite) > get_player_min_x()) move_gfx_animated_sprite_physic(&player_sprite, -get_player_speed(), 0);
  if (joypad_is_right(0) && get_x_gfx_animated_sprite_physic(player_sprite) < get_player_max_x()) move_gfx_animated_sprite_physic(&player_sprite, get_player_speed(), 0);

  if (!joypad_is_down(0) && !joypad_is_up(0)) set_animation_gfx_animated_sprite_physic(&player_sprite, PLAYER_IMG_ANIM_IDLE);
  bullet_player_update(joypad_is_a(0), get_x_gfx_animated_sprite_physic(player_sprite), get_y_gfx_animated_sprite_physic(player_sprite));
}
