...
int main(void) {
  ...
  image_display(&logo1, &logo_sprite_Palettes, 10, 20);
  logger_set_position(1, 10);
  logger_info("VERTICAL SHRUNK");
  image_display(&logo2, &logo_sprite_Palettes, 10, 80);
  logger_set_position(1, 19);
  logger_info("PROPORTIONAL SHRUNK");
  image_display(&logo3, &logo_sprite_Palettes, 10, 150);
  ...
  while(1) {
    ...
    image_shrunk(&logo3, shrunk_prop_table_get(DAT_frameCounter & SHRUNK_TABLE_PROP_SIZE)); /* neocore provide a precalculated table for keep "aspect ratio" */
    SCClose();
  };
  SCClose();
  return 0;
