Import-Module "$($Config.project.neocorePath)\toolchain\scripts\modules\services\writers\nsi.ps1"
...
function BuilderEXE {
  ...
  if ((Test-Path -Path "$distPath\$packageName\$packageName-$version\exe\$packageName-$version.exe") -eq $true) {
    Write-Host ""
    Write-Host "builded exe is available at $distPath\$packageName\$packageName-$version\exe\$packageName-$version.exe" -ForegroundColor Green
  }
}
