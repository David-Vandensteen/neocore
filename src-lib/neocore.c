...
static void neocore_asprite_physic_show_toggle(aSpritePhysic *asp);
...
static void neocore_asprite_physic_show_toggle(aSpritePhysic *asp) {
  asp->visible = !asp->visible;
  aSpritePhysicShow(asp, asp->visible);
}
...
void aSpritePhysicShow(aSpritePhysic *asp, BOOL pvisible) {
  switch(pvisible) {
    case true:
    default:
    aSpriteShowNeocore(&asp->as, true);
    //neocore_gpu_asprite_show(&asp->as, true);
    asp->visible = true;
    break;

    case false:
    aSpriteShowNeocore(&asp->as, false);
    //aSpriteHide(&asp->as);
    //neocore_gpu_asprite_show(&asp->as, false);
    asp->visible = false;
    break;
  }
}
...
void aSpritePhysicFlash(aSpritePhysic *asp, BOOL pflash, WORD freq) {
  asp->flashing = pflash;
  asp->flashingFreq = freq;
  if (!asp->flashing) aSpritePhysicShow(asp, true);
}

void aSpritePhysicFlashUpdate(aSpritePhysic *asp) {
  if (asp->flashing && DAT_frameCounter % asp->flashingFreq == 0) neocore_asprite_physic_show_toggle(asp);
}
